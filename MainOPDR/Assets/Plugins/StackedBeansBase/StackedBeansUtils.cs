...
namespace StackedBeans.Utils
{
    public static class StackedBeansUtils
    {
        ...
        public static async UniTask<TextMeshPro> CreateWorldTextMeshPro(Transform parent, string text,
            Vector3 worldPosition,Vector3 worldRotation, Color color, int fontSize = 36, TextAnchor textAnchor = TextAnchor.MiddleCenter,
            TextAlignmentOptions textAlignment = TextAlignmentOptions.Center, int sortingOrder = 5000)
        {
            GameObject gameObject = await CreateAdressableObject("BaseText", parent, worldPosition,worldRotation);
            ...
            Vector3 worldPosition, Vector3 worldRotation, int scale = 1, int sortingOrder = 5000)
        {
            ...
            GameObject gameObject = await CreateAdressableObject("BaseSprite", parent, worldPosition,worldRotation);
        ...
        public static async UniTask<GameObject> CreateAdressableObject(string AdressableName, Transform parent, Vector3 worldPosition,Vector3 worldRotation)
        {
            GameObject gameObject = await Addressables.LoadAssetAsync<GameObject>(AdressableName).Task;
        ...
        public static async UniTask<ScriptableObject> GetAdressableScriptableObject(string AdressableName)
        {
            ScriptableObject scriptableObject = await Addressables.LoadAssetAsync<ScriptableObject>(AdressableName).Task;
